...
class TestNodePopulationNodeSet(unittest.TestCase):
    ...
    def test_BasicScalarAnded(self):
        j = json.dumps({"NodeSet0": {"E-mapping-good": "C", "attr-Y": [21, 22]}})
        sel = NodeSets(j).materialize("NodeSet0", self.population)
        self.assertEqual(sel, Selection(((0, 1), )))

    def test_BasicScalarNodeId(self):
        sel = NodeSets(json.dumps({"NodeSet0": {"node_id": [1, 3, 5]}})).materialize("NodeSet0", self.population)
        self.assertEqual(sel, Selection(((1, 2), (3, 4), (5, 6))))

        sel = NodeSets(json.dumps({"NodeSet0": {"node_id": [1, 3, 500]}})).materialize("NodeSet0", self.population)
        self.assertEqual(sel, Selection(((1, 2), (3, 4), )))

    def test_BasicScalarPopulation(self):
        sel = NodeSets(json.dumps({"NodeSet0": {"population": "nodes-A"}})).materialize("NodeSet0", self.population)
        self.assertEqual(sel, self.population.select_all())

        sel = NodeSets(json.dumps({"NodeSet0": {"population": ["nodes-A", "FAKE"]}})).materialize("NodeSet0", self.population)
        self.assertEqual(sel, self.population.select_all())
    ...
    def test_NodeSetEmptyArray(self):
        sel = NodeSets(json.dumps({"NodeSet0": {"node_id": []}})).materialize("NodeSet0", self.population)
        self.assertEqual(sel, Selection([]))

        sel = NodeSets(json.dumps({"NodeSetCompound0": []})).materialize("NodeSetCompound0", self.population)
        self.assertEqual(sel, Selection([]))
    ...
    def test_extend(self):
        ...
        ns = NodeSets(json.dumps({"NodeSet0": {"attr-Y": [21, 22]}}))
